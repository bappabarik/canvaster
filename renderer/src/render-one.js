...
if (!projectId || isNaN(projectId)) {
    console.error('Usage: node src/render-one.js --project=<id> [--row=<rowIndex>] [--out=<path>]');
    process.exit(1);
}
...
function safeJsonParse(value, fallback) {
    ...
    try {
        ...
        console.error('  Raw value (first 200 chars):', String(value).slice(0, 200));
        return fallback;
    }
}
...
async function main() {
    ...
    const db = await mysql.createConnection({
        ...
        typeCast(field, next) {
            if (
                field.type === 'JSON' ||
                ['data', 'column_map', 'placeholders_snapshot', 'canvas_snapshot_json'].includes(field.name)
            ) {
    ...
    try {
        ...
        const canvasJson   = typeof project.canvas_snapshot_json === 'string'
        ...
        console.log('  canvas     :', project.width_px + 'x' + project.height_px + 'px');
        ...
        if (!canvasJson || canvasJson === 'null') {
            throw new Error('canvas_snapshot_json is empty — was the project created properly?');
        }
        ...
        for (const [placeholder, csvHeader] of Object.entries(columnMap)) {
            ...
            if (value && imageExtensions.test(String(value))) {
                ...
                if (assets.length) {
                    ...
                    console.warn(`  imageMap   : ${value} → ⚠ not found in uploaded_assets (will render blank)`);
                }
            }
        }
